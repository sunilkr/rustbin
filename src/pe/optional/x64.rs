...
impl Header for OptionalHeader64 {
    fn parse_bytes(bytes: &[u8], pos: u64) -> crate::Result<Self> where Self: Sized {
        ...
        hdr.sizeof_stack_reserve = new_header_field!(cursor.read_u64::<LittleEndian>()?, offset);
        hdr.sizeof_stack_commit = new_header_field!(cursor.read_u64::<LittleEndian>()?, offset);
        hdr.sizeof_heap_reserve = new_header_field!(cursor.read_u64::<LittleEndian>()?, offset);
        hdr.sizeof_heap_commit = new_header_field!(cursor.read_u64::<LittleEndian>()?, offset);
...
mod tests {
    ...
    #[test]
    fn test_json() {
        let opt = OptionalHeader64::parse_bytes(&RAW_BYTES, 0x128).unwrap();
